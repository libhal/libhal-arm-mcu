 ...
 * Example usage:
 * ```
 * auto ptr = hal::make_strong_ptr<MyClass>(allocator,
 *                                          std::move(obj.get_my_class));
 * ```
 *
 * @tparam T The type of object to create
 * @param p_alloc Allocator to use for memory allocation
 * @param p_temporary Temporary object to passed to move constructor
 * @return A strong_ptr managing the newly created object
 */
template<class T>
[[nodiscard]] inline strong_ptr<T> make_strong_ptr(
...
{
  using rc_t = detail::rc<T>;
  auto* obj = p_alloc.new_object<rc_t>(p_alloc, std::move(p_temporary));
  return strong_ptr<T>(&obj->m_info, &obj->m_object);
}
}  // namespace hal::v5
