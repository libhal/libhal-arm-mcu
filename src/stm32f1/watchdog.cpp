...
#include <cstdint>
#include <libhal-arm-mcu/stm32f1/watchdog.hpp>
...
class watch_dog {
  virtual void start_independent_watchdog()
  {
    iwdg_regs->set_kr(0xCCCC);
  }
  virtual void reset_independent_watchdog_counter()
  {
    iwdg_regs->set_kr(0xAAAA);
  }

  virtual void set_independent_watchdog_countdown_time(hal::time_duration p_wait_time)
  {
  ...
  virtual bool check_independent_watchdog_flag()
  {
    // sec. 8.3.10, pg. 152
    return bit_extract(bit_mask::from(29), *reset_status_register);
  }

  virtual void clear_reset_flags()
  {
    // sec. 8.3.10, pg. 152
