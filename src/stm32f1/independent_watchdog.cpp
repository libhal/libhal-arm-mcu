...
struct independent_watchdog_registers
{
  ...
  void set_kr(uint32_t p_value)
  {
    ...
    bit_modify(kr).insert<writeable_kr>(p_value);
  }
  void set_pr(uint32_t p_value)
  {
    constexpr bit_mask writeable_pr = bit_mask::from(0, 2);
    bit_modify(pr).insert<writeable_pr>(p_value);
  }
  void set_rlr(uint32_t p_value)
  {
    constexpr bit_mask writeable_rlr = bit_mask::from(0, 11);
    bit_modify(rlr).insert<writeable_rlr>(p_value);
  }
};
