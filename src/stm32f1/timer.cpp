  ...
  pin_type p_pin)
{
  ...
  return this->acquire_pwm(static_cast<pins>(p_pin),
                           pwm_timer1,
                           stm32f1::frequency(peripheral::timer1));
}
  ...
  pin_type p_pin)
{
  ...
  if constexpr (select == peripheral::timer2) {
    return this->acquire_pwm(static_cast<pins>(p_pin),
                             pwm_timer2,
                             stm32f1::frequency(peripheral::timer2));
  } else if constexpr (select == peripheral::timer3) {
    return this->acquire_pwm(static_cast<pins>(p_pin),
                             pwm_timer3,
                             stm32f1::frequency(peripheral::timer3));
  } else {
    return this->acquire_pwm(static_cast<pins>(p_pin),
                             pwm_timer4,
                             stm32f1::frequency(peripheral::timer4));
  }
}
