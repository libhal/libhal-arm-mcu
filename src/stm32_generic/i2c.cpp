...
enum class i2c_state : hal::u32
{

  write_data_byte = bit_value().set(i2c_sr1::tx_empty).to<hal::u32>(),
  write_read_addr = bit_value()
                      .set(i2c_sr1::byte_transfered_finish)
                      .set(i2c_sr1::tx_empty)
                      .to<hal::u32>(),
  read_new_byte = bit_value()
                    .set(i2c_sr1::byte_transfered_finish)
                    .set(i2c_sr1::rx_not_empty)
                    .to<hal::u32>(),
  byte_transfer_success =
    bit_value().set(i2c_sr1::byte_transfered_finish).to<hal::u32>(),
  stop = bit_value().set(i2c_sr1::stop_detection).to<hal::u32>(),
  addr10 = bit_value().set(i2c_sr1::addr10).to<hal::u32>(),
  addr_rd = bit_value().set(i2c_sr1::addr).to<hal::u32>(),
  addr_wr =
    bit_value().set(i2c_sr1::addr).set(i2c_sr1::tx_empty).to<hal::u32>(),
  start = bit_value().set(i2c_sr1::start).to<hal::u32>(),
};
enum class i2c_error : hal::u32
{
  bus_error = bit_value().set(i2c_sr1::bus_error).to<hal::u32>(),
  arbitration_lost = bit_value().set(i2c_sr1::arbitration_lost).to<hal::u32>(),
  ack_failure = bit_value().set(i2c_sr1::ack_failure).to<hal::u32>(),
  overrun_underrun = bit_value().set(i2c_sr1::over_under_run).to<hal::u32>(),
  timeout = bit_value().set(i2c_sr1::timeout_error).to<hal::u32>(),

};
enum class addr_state : hal::u32
{
  timeout = bit_value().set(i2c_sr2::trans_reciever).to<hal::u32>(),
  nothing = 0,
};
enum class i2c_control
{
  start,
  stop
};
...
void i2c::configure(hal::i2c::settings const& p_settings, hertz p_frequency)
{
  ...
  if (p_frequency < 2_MHz || p_frequency > 50_MHz) {
  ...
  if (p_settings.clock_rate > slow_mode_max_speed) {
    u16 const ccr_value =
      static_cast<hal::u8>(p_frequency / (3 * p_settings.clock_rate));
    bit_modify(i2c_reg->ccr)
      .set(i2c_ccr::i2c_mode_sel)
      .insert(i2c_ccr::ccr, ccr_value);
  } else {
    u16 const ccr_value =
      static_cast<hal::u8>(p_frequency / (2 * p_settings.clock_rate));
    bit_modify(i2c_reg->ccr)
